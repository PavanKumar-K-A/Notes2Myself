...
FORMATTED_CC_EMAIL_IDS = [                                                          # List of CC Email Ids
    formataddr((TO_EMAIL_ID_3_SENDER, TO_EMAIL_ID_3))
]
FORMATTED_BCC_EMAIL_IDS = [                                                         # List of BCC Email Ids
    formataddr((TO_EMAIL_ID_2_SENDER, TO_EMAIL_ID_2))
]
message['CC'] = ', '.join(FORMATTED_CC_EMAIL_IDS)                   # Add CC email ids in the message and the envelope.
# message['BCC'] = ', '.join(TO_BCC_EMAIL_IDS)                      # Add BCC email address ONLY in the envelope. If BCC
                                                                    # email address is put into the message then it will
...
server.sendmail(FROM_EMAIL_ID,
                FORMATTED_TO_EMAIL_IDS +
                FORMATTED_CC_EMAIL_IDS +
                FORMATTED_BCC_EMAIL_IDS,
                text)                                                               # Add TO, CC, BCC in envelope.
server.quit()
